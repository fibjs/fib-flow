...
- Use stable, readable strings for `task.audit()` checkpoint codes and `task.progress().stage_name`; lowercase `snake_case` is recommended for consistency, but not required
