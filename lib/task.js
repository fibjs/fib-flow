...
class TaskManager {
        ...
        if (typeof handler === 'function') {
            ...
            if (parameters) {
                ...
                if (!validateResult.success) {
                    logger.error(`[TaskManager] ${validateResult.error}`);
    ...
    async(taskName, payload = {}, options = {}) {
        ...
        if (registeredTask.validator) {
            ...
            if (!valid) {
                ...
                logger.error(`[TaskManager] Payload validation failed for task ${taskName}: ${errorMsg}`);
    ...
    getTaskInfo(taskName) {
        ...
        if (!this.handlers.has(taskName)) {
            logger.error(`[TaskManager] No task info found for: ${taskName}`);
