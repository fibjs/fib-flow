...
class TaskManager {
    ...
    use(taskName, handler) {
        ...
        if (typeof handler === 'function') {
            ...
            taskOptions = { max_retries, retry_interval, timeout, priority };
        } else {
    ...
    pauseTask(taskId) {
        ...
        if (this.state !== 'running') {
            logger.error(`[TaskManager] Cannot pause task in state: ${this.state}`);
            throw new Error('Can only pause task when TaskManager is running');
        }
    ...
    _executeTask(task) {
        ...
        coroutine.start(async () => {
            try {
                ...
                const result = await this.handlers.get(task.name).handler(task, tasks => new SubTasks(tasks));
                if (this.state !== 'running') {
    ...
    start() {
        ...
        coroutine.start(() => {
            ...
            while (this.state === 'running') {
                ...
                const task = this.db.claimTask(Array.from(this.handlers.keys()), this.options.worker_id);
                if (!task) {
    ...
    deleteTasks(filters = {}) {
        ...
        if (this.state !== 'running') {
            logger.error(`[TaskManager] Cannot delete tasks in state: ${this.state}`);
            throw new Error('Can only delete tasks when TaskManager is running');
        }
