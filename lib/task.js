 ...
 * @param {string} path - Current path in the schema for error reporting
 * @returns {Object} Validation result with success flag and error message or validator
 */
function validateSchema(schema, path) {
    ...
    try {
        ...
        return {
            success: false,
            error: `Invalid schema at ${path}: ${e.message}`
        };
    }
...
function createExecutionEntry({ handler, validator, options }) {
    return {
        handler,
        validator,
...
class TaskManager {
    ...
    * @param {number} options.worker_ttl - Worker liveness TTL in milliseconds
    ...
    * Register task handlers for specific task types.
    ...
    use(taskName, handler) {
        ...
        if (typeof handler === 'function') {
            ...
            // Validate parameters and output schema if provided
            if (parameters) {
                const validateResult = validateSchema(parameters, `${taskName}.parameters`);
            ...
            if (output) {
                const validateResult = validateSchema(output, `${taskName}.output`);
                if (!validateResult.success) {
                    throw new Error(validateResult.error);
                }
            }
    ...
    async(taskName, payload = {}, options = {}) {
        ...
        // Validate payload against schema if a validator exists
        if (registeredTask.validator) {
            const valid = registeredTask.validator(payload);
            if (!valid) {
                const errors = registeredTask.validator.errors;
                const errorMsg = errors.map(e => `${e.instancePath} ${e.message}`).join('; ');
                throw new Error(`Invalid task payload: ${errorMsg}`);
            }
            logger.info(`[TaskManager] Payload validation passed for task ${taskName}`);
        }
    ...
    runRetention(policy = null) {
        ...
        return this.db.cleanupExpiredTasks(effectivePolicy);
    }
    ...
    _registerWorkerLifecycle() {
        ...
        const now = Math.floor(Date.now() / 1000);
        const workerTtlSeconds = Math.max(1, Math.ceil(this.options.worker_ttl / 1000));
    ...
    _startWorkerHeartbeat() {
        ...
        const interval = this.options.worker_heartbeat_interval || this.options.active_update_interval;
        const workerTtlSeconds = Math.max(1, Math.ceil(this.options.worker_ttl / 1000));
        this.workerTimer = setInterval(() => {
            const now = Math.floor(Date.now() / 1000);
            this.db.heartbeatWorker(this.options.worker_id, now, workerTtlSeconds);
            const expiredWorkerIds = this.db.reapExpiredWorkers(now);
            if (this.options.recover_running_jobs && expiredWorkerIds.length > 0) {
                this.db.recoverTasksForWorkers(expiredWorkerIds, {
                    worker_id: this.options.worker_id,
                    pod_id: this.options.pod_id,
                    now
                });
    ...
    _isCurrentWorkerActive() {
        ...
        const worker = this.db.getWorker(this.options.worker_id);
        const isActive = Boolean(worker && worker.status === 'active');

        if (isActive !== this.workerActiveState) {
            this.workerActiveState = isActive;
            if (!isActive) {
                logger.warning(`[TaskManager] Worker ${this.options.worker_id} is no longer active; skipping task claims`);
            }
        }
            ...
            try {
                ...
                if (result instanceof SubTasks) {
                    ...
                    for (const childTask of result.tasks) {
                        ...
                        // If validator exists, validate child task payload
                        if (registeredChildTask.validator && childTask.payload) {
                            const valid = registeredChildTask.validator(childTask.payload);
                            if (!valid) {
                                const errors = registeredChildTask.validator.errors;
                                const errorMsg = errors.map(e => `${e.instancePath} ${e.message}`).join('; ');
                                throw new Error(`Invalid child task payload for task ${childTask.name}: ${errorMsg}`);
                            }
                            logger.debug(`[TaskManager] Child task payload validation passed for task ${childTask.name}`);
                        }
                    }
    ...
    start() {
        ...
        this.activeTimer = setInterval(() => {
            ...
            this.db.handleTimeoutTasks(this.options.active_update_interval, this.options.retention);
     ...
     * @returns {Object} Task definition including name, description, parameters schema, and output schema
    ...
    getTaskInfo(taskName) {
        ...
        return {
            ...
            parameters: taskConfig.options.parameters || {},
            output: taskConfig.options.output || {}
        };
    }
