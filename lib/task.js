...
/**
 * Container class for explicitly suspending a task for external interaction
 * (e.g. human approval). Returning this from a task handler transitions the
 * task to 'suspended' status. The task can later be resumed via
 * TaskManager#resumeTask(taskId, { data }); the handler then re-runs and can
 * read task.resume_data to continue.
 */
class Suspension {
    /**
    ...
    resumeTask(taskId, options = {}) {
        ...
        if (options && typeof options === 'object') {
            if ('data' in options && options.data !== undefined) {
                extra.resume_data = options.data;
            }
            if (options.resume_reason) {
