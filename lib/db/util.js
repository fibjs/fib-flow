...
function parseTask(task) {
    ...
    // Parse the resume data if it's stored as string
    if (typeof task.resume_data === 'string') {
        try {
            task.resume_data = JSON.parse(task.resume_data);
        } catch (e) {
            // Keep original resume data if parsing fails
        }
    }
