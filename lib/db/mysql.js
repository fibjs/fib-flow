        ...
        this.pool(conn => {
            conn.execute(`
                CREATE TABLE IF NOT EXISTS dcron_tasks (
                    ...
                    -- 索引和约束
                    KEY idx_dcron_tasks_status_priority_next_run_time (status, priority, next_run_time),
                    KEY idx_dcron_tasks_name (name),
                    KEY idx_dcron_tasks_parent_status (parent_id, status)
