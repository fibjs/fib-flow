...
class BaseDBAdapter {
        ...
        const resumeRs = conn.execute(
            `UPDATE fib_flow_tasks
              ...
              WHERE id = ? AND status = 'suspended'
                AND (suspend_reason IS NULL OR suspend_reason = 'awaiting_subtasks')
    ...
    updateTaskStatus(taskId, status, extra = {}) {
        ...
        this.pool(conn => {
            ...
            // Explicit suspension metadata (task.suspend / resumeTask)
            if ('suspend_reason' in extra) {
                updates.push('suspend_reason = ?');
                params.push(extra.suspend_reason === undefined || extra.suspend_reason === null ? null : extra.suspend_reason);
            }
            ...
            conn.trans(() => {
                ...
                if (status === 'pending') {
                    if (resumingFromSuspended) {
                        ...
                        updates.push('suspend_reason = NULL');
                    } else {
    ...
    * @param {object} timeoutConfig - Task heartbeat timeout configuration
    ...
    getTasksByStatus(status, filters = {}) {
        ...
        if (filters.suspend_reason) {
            conditions.push('suspend_reason = ?');
            params.push(filters.suspend_reason);
        }

        return this.pool(conn => {
            const tasks = conn.execute(
                `SELECT * FROM fib_flow_tasks WHERE ${conditions.join(' AND ')}`,
                ...params
            );
            return tasks.map(task => parseTask(task));
        });
