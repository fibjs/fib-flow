...
class BaseDBAdapter {
     ...
     */
    claimTask(taskNames) {
        logger.info(`[claimTask] Attempting to claim task for names:`, taskNames);
        ...
        try {
            this.pool(conn => {
                ...
                conn.trans(() => {
                    ...
                    if (rs.affected === 0) {
                        logger.error(`[updateTaskStatus] Failed to update task ${taskId} - no rows affected`);
     ...
     */
    handleTimeoutTasks() {
        logger.info(`[handleTimeoutTasks] Starting timeout task handling`);
        const now = Math.floor(Date.now() / 1000);

        try {
            return this.pool(conn => conn.trans(() => {
                // Set remaining timed-out tasks to 'timeout'
                logger.debug(`[handleTimeoutTasks] Checking for timed-out tasks`);
                ...
                if (timeoutRs.affected) {
                    logger.warning(`[handleTimeoutTasks] ${timeoutRs.affected} tasks timed out`);
                }
