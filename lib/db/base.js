...
class BaseDBAdapter {
    ...
    _cleanupExpiredTasksWithConnection(conn, policy) {
        ...
        const deletedTasks = conn.execute('DELETE FROM fib_flow_tasks WHERE id IN ?', expiredTaskIds);

        return {
            tasks_deleted: deletedTasks && deletedTasks.affected ? deletedTasks.affected : 0,
    ...
    recordTaskProgress(taskId, progress = {}) {
        if (!taskId) {
            throw new Error('Task ID is required');
        }
    ...
    recordTaskCheckpoint(taskId, checkpoint = {}) {
        if (!taskId) {
            throw new Error('Task ID is required');
        }
     ...
     */
    handleTimeoutTasks(active_update_interval, retention = null) {
        ...
        return this.pool(conn => {
            ...
            const totalTimeoutTasks = conn.execute(
                `SELECT id, name, stage, retry_count, root_id, parent_id, worker_id FROM fib_flow_tasks
                ...
                const rs = conn.execute(
                    `UPDATE fib_flow_tasks
                         SET status = 'timeout',
                             ...
                             last_active_time = ?
                         WHERE id = ? AND status = 'running'`,
                    'Task heartbeat lost - worker may be dead', now, task.id
                );

                if (rs.affected === 1) {
                    heartbeatTimeoutAffected += 1;
                    this._finishOpenTaskAttemptWithConnection(conn, task.id, {
                        ended_at: now,
                        outcome: 'timeout',
            ...
            if (exhaustedAffected) {
                logger.warning(`[handleTimeoutTasks] ${exhaustedAffected} cron tasks paused due to no retries left`);
            }
                ...
                this.pool(conn => {
                    conn.trans(() => {
                        ...
                        if (rs.affected === 1 && task.parent_id) {
                            ...
                            this._resumeParentTaskWithConnection(conn, task.parent_id, `${task.id}!${JSON.stringify(task.error)}\n`, now);
            ...
            if (retentionPolicy.expire_time) {
                const cleanupResult = this._cleanupExpiredTasksWithConnection(conn, {
                    ...retentionPolicy,
                    now
                });
    ...
    cleanupExpiredTasks(policy = {}) {
        ...
        return this.pool(conn => {
            let cleanupResult;
            conn.trans(() => {
                cleanupResult = this._cleanupExpiredTasksWithConnection(conn, retentionPolicy);
            });
