...
class BaseDBAdapter {
    ...
    _handleHeartbeatTimeoutTasks(now, active_update_interval) {
                ...
                const rs = conn.execute(
                    `UPDATE fib_flow_tasks
                         SET status = 'timeout',
                             ...
                             last_active_time = ?
                         WHERE id = ? AND status = 'running'`,
                    'Task heartbeat lost - worker may be dead',
                    now,
                    task.id
                );
    ...
    handleTimeoutTasks(active_update_interval, retention = null) {
        ...
        const result = {
            ...
            timed_out_heartbeat: this._handleHeartbeatTimeoutTasks(now, active_update_interval),
        ...
        if (result.timed_out_heartbeat) {
            logger.warning(`[handleTimeoutTasks] ${result.timed_out_heartbeat} tasks lost heartbeat`);
        }
        if (result.retried) {
