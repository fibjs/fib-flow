    ...
    setup() {
        this.pool(conn => {
            conn.execute(`
                ...
                CREATE INDEX IF NOT EXISTS idx_status_priority_next_run_time ON dcron_tasks(status, priority, next_run_time);
                CREATE INDEX IF NOT EXISTS idx_name ON dcron_tasks(name);
            `);
        });
    }
