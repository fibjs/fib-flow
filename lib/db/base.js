...
class BaseDBAdapter {
    ...
    handleTimeoutTasks(active_update_interval, expireTime = null) {
        ...
        return this.pool(conn => {
            ...
            if (pausedRs.affected) {
                logger.warning(`[handleTimeoutTasks] ${pausedRs.affected} cron tasks paused due to no retries left`);
            }
                ...
                this.pool(conn => {
                    conn.trans(() => {
                        ...
                        if (rs.affected === 1 && task.parent_id) {
                            logger.info(`[updateTaskStatus] Updating parent task ${task.parent_id}`);
                            conn.execute(`
                                UPDATE fib_flow_tasks
                                SET
                                    completed_children = completed_children + 1,
                        ...
                        `, `${task.id}!${task.error}\n`, task.parent_id, task.parent_id);
                            logger.info(`[updateTaskStatus] Parent task update completed for ${task.parent_id}`);
                        }
                    });
                });
