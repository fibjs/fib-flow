...
class MemoryAdapter extends BaseDBAdapter {
    ...
    insertTask(tasks, options = {}) {
        ...
        try {
            ...
            const originalTasks = new Map(this.tasks);
            const originalIndexes = JSON.parse(JSON.stringify(Object.fromEntries(
                Object.entries(this.indexes).map(([key, map]) => [
                    key, 
                    Object.fromEntries(Array.from(map.entries()).map(([k, set]) => [k, Array.from(set)]))
                ])
            )));
    ...
    claimTask(taskNames, workerId) {
        ...
        if (clonedTask.payload) {
            try {
                clonedTask.payload = JSON.parse(clonedTask.payload);
            } catch (error) {
                logger.error(`[MemoryAdapter] Failed to parse payload for task ${task.id}:`, error);
            }
        }
    ...
    handleTimeoutTasks(active_update_interval, expireTime = null) {
        ...
        for (const taskId of failedIds) {
            ...
            if (task.retry_count >= task.max_retries) {
                // Mark as permanently failed or paused for cron
                task.status = task.type === 'cron' ? 'paused' : 'permanently_failed';
                task.last_active_time = now;
                updated = true;
                permanentlyFailedCount++;
            } else if (task.retry_count < task.max_retries && 
                       task.last_active_time + task.retry_interval < now) {
                // Retry the task
                task.status = 'pending';
                task.retry_count += 1;
                task.last_active_time = now;
        ...
        while (hasChanges) {
            hasChanges = false;
            const suspendedIds = this.indexes.byStatus.get('suspended') || new Set();
            
            for (const taskId of suspendedIds) {
                const task = this.tasks.get(taskId);
                if (!task) continue;

                // Check if any child task is permanently failed
                const childIds = this.indexes.byParent.get(taskId) || new Set();
                const hasFailedChild = Array.from(childIds).some(childId => {
                    const childTask = this.tasks.get(childId);
                    return childTask && childTask.status === 'permanently_failed';
                });
        ...
        if (totalTimeoutCount || heartbeatTimeoutCount || retryCount || 
            permanentlyFailedCount || workflowFailedCount || expiredCount) {
            logger.info(`[MemoryAdapter] Timeout handling complete: ` +
                ...
                `retry=${retryCount}, permanently_failed=${permanentlyFailedCount}, ` +
                `workflow_failed=${workflowFailedCount}, expired=${expiredCount}`);
        }
    }
    ...
    getTask(taskId) {
        ...
        const task = this.tasks.get(taskId);
        return task ? this._parseTask({ ...task }) : null;
    }
    ...
    getTasksByName(name) {
        ...
        for (const taskId of taskIds) {
            const task = this.tasks.get(taskId);
            if (task) {
                tasks.push(this._parseTask({ ...task }));
            }
        }
    ...
    /**
     * Parse task data (override for memory-specific handling)
     * @private
     * @param {object} task - Task to parse
    ...
    _parseTask(task) {
        ...
        if (task.result && typeof task.result === 'string') {
            try {
                ...
                logger.error(`[MemoryAdapter] Failed to parse result for task ${task.id}:`, error);
            }
        }
