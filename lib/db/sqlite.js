    ...
    setup() {
        this.pool(conn => {
            conn.execute(`
                ...
                -- 任务调度索引
                CREATE INDEX IF NOT EXISTS idx_dcron_tasks_status_priority_next_run_time ON dcron_tasks(status, priority, next_run_time);
                CREATE INDEX IF NOT EXISTS idx_dcron_tasks_name ON dcron_tasks(name);
                CREATE INDEX IF NOT EXISTS idx_dcron_tasks_parent_status ON dcron_tasks(parent_id, status);
            `);
        });
    }
}
